...
public class Lab5 extends GLJPanel implements GLEventListener, KeyListener{
	...
	private void draw(GL2 gl2) {
		...
		if(objectNumber==1) //KorkociÂąg
		{
			...
			float x,z;
			float y = -9; //WysokoĹ“Ă¦ na poczÂątku
			float i;
			int n = 11;
			...
			for (i = 0.0f; i<= (java.lang.Math.PI * 2f * n); i += distance) {
				...
				gl2.glColor3f(0.0f, 0.0f, 0.05f); //darkblue
				gl2.glVertex3f(x, y, z);
				y += 0.08; //PowiĂŞkszenie
			}
			gl2.glEnd();
		}
...
//Boki piramidy
			...
			gl2.glEnd();
			gl2.glBegin(GL2.GL_TRIANGLES); 
				//WielokÂątÂąt o podstawie n =11
				for (i = 0; i<= n*2; i += 1) {
					x = (float) Math.sin(i * 2 * Math.PI / n)*rozmiar;
					z = (float) Math.cos(i * 2 * Math.PI / n)*rozmiar;
					gl2.glColor3f(1.0f, 1.0f, 1.0f);  //BiaÂły kolor podstawy
					gl2.glVertex3f(x, y, z);
					if(i%2==0) {
